 ...
 * Fix to 'use' logic: don't overwrite .terragrunt-version files
 ...
 * Introduce tgenv-resolve-version to deduplicate translation of requested version into actual version
 * README.md updates
 * Fix #176 - New parameter TGENV_AUTO_INSTALL to handle the version specified by `use` or a `.terragrunt-version` file not being installed

## 1.0.2 (October 29, 2019)

 * Fix failing 0.11.15-oci version, Add additional tests for 0.11.15-oci and alphas, betas and rcs #145
 * Fix to README section link for .terragrunt-version file #146
 ...
 * run tgenv as a neighbour with full path (to keep vscode and whoever doesn't respect %path, happy) #72
 * Add current version in list command #69
 * Version file #67
 * [DOC] Add link to puppet module for automatic tgenv installation #64
 ...
 * Upgrade tgenv-install logging
 * Prevent interactive prompting from keybase

## 0.4.2 (April 9, 2017)

 * Add support for verifying downloads of Terragrunt

## 0.4.1 (March 8, 2017)

 * Update error_and_die functions to better report their source location
 * libexec/tgenv-version-name: Respect `latest` & `latest:<regex>` syntax in .terragrunt-version
